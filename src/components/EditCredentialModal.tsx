...
export function EditCredentialModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="modal-edit-credential-title"
      onMouseDown={(event) => {
    ...
    >
      <div className="w-full max-w-lg rounded-2xl border border-slate-700/80 bg-slate-950 p-6 shadow-2xl">
        <h2
          id="modal-edit-credential-title"
          className="font-['Sora',ui-sans-serif,system-ui,sans-serif] text-xl font-bold text-white"
        >
          Editar credencial
        </h2>
        <p className="mt-1 text-sm text-slate-400">
          Los cambios se guardan en el apartado actual.
        </p>
        <form className="mt-6 space-y-4" onSubmit={handleFormSubmit}>
          <div>
            <label htmlFor="modal-edit-cred-title" className="text-xs text-slate-400">
              Cuenta o servicio
            </label>
            <input
              ref={firstInputRef}
              id="modal-edit-cred-title"
              value={title}
          ...
          <div>
            <label htmlFor="modal-edit-cred-user" className="text-xs text-slate-400">
              Usuario o correo
            </label>
            <input
              id="modal-edit-cred-user"
              value={username}
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <label htmlFor="modal-edit-cred-pass" className="text-xs text-slate-400">
                Contraseña
              </label>
              <button
                type="button"
